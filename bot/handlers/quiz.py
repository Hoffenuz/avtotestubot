...
from bot.services.quiz import send_next_question, send_quiz_poll
from bot.services.quiz_session import end_session, get_session, start_session
...
@router.message(F.text == "📝 Tez test")
async def start_quick_quiz(message: Message) -> None:
    ...
    if get_session(user_id):
        await message.answer("⚠️ Avval joriy testni tugating yoki /stop buyrug'ini yuboring.")
        return

    lang = await _get_lang(user_id)
    session = start_session(user_id, "quick", lang)
    await message.answer(
        f"📝 <b>Tez test boshlandi!</b>\n"
        f"Savollar soni: {len(session.questions)}\n\n"
        "Har bir savol quiz ko'rinishida yuboriladi.",
        parse_mode="HTML",
    )
    await send_quiz_poll(message.bot, user_id, session)


@router.message(F.text == "🎯 Imtihon")
async def start_exam(message: Message) -> None:
    ...
    user_id = message.from_user.id
    if get_session(user_id):
        await message.answer("⚠️ Avval joriy testni tugating.")
        return

    lang = await _get_lang(user_id)
    session = start_session(user_id, "exam", lang)
    await message.answer(
...
async def restart_exam(callback: CallbackQuery) -> None:
    ...
    if callback.message:
        ...
        await send_quiz_poll(callback.bot, user_id, session)


@router.message(F.text == "📚 Bilet bo'yicha")
...
async def start_ticket_quiz(callback: CallbackQuery) -> None:
    ...
    if get_session(user_id):
        await callback.answer("Avval joriy testni tugating!", show_alert=True)
        return

    lang = await _get_lang(user_id)
    session = start_session(user_id, "ticket", lang, ticket_num=ticket_num)
...
async def next_question(callback: CallbackQuery) -> None:
    ...
    user_id = callback.from_user.id
    sent = await send_next_question(callback.bot, user_id)
    if sent:
        await callback.answer()
    else:
        await callback.answer("Test tugagan yoki mavjud emas.", show_alert=True)
